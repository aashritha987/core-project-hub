...
export default function Reports() {
  ...
  const metrics = useMemo(() => {
    ...
    const sprintCompletionPct = completionScopeIssues.length > 0
      ...
      : 0;
    const sprintTotalPoints = completionScopeIssues.reduce((sum, i) => sum + (i.storyPoints || 0), 0);
    const sprintDonePoints = completionScopeIssues
    ...
    const unestimatedInSprint = activeSprintIssues.filter((i) => i.storyPoints == null).length;
    ...
    const healthScore = Math.max(
      ...
      Math.min(
        100,
        40
        + Math.round(sprintCompletionPct * 0.3)
        + Math.round(deliveryPredictability * 0.3)
        - overdueOpen.length * 3
        - Math.max(0, reviewWip - 3) * 2
        - Math.max(0, unassignedOpen.length - 2),
      ),
    );
    ...
    if (unestimatedInSprint > 0) {
      recommendations.push(`${unestimatedInSprint} sprint issues have no estimate. Add story points for better predictability.`);
    }
  ...
  const healthTone = metrics.healthScore >= 75
    ? 'bg-jira-green-light text-jira-green border-jira-green/30'
    : metrics.healthScore >= 50
      ? 'bg-jira-yellow-light text-jira-orange border-jira-orange/30'
      : 'bg-jira-red-light text-jira-red border-jira-red/30';
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="border shadow-sm">
          <CardContent className="pt-4 pb-4 px-4">
            <p className="text-2xs text-muted-foreground font-medium">Project Health Score</p>
            <div className="mt-2 flex items-center gap-2">
              <p className="text-2xl font-bold text-foreground">{metrics.healthScore}</p>
              <Badge variant="outline" className={healthTone}>/100</Badge>
            </div>
            <p className="text-2xs text-muted-foreground mt-1">Combines sprint progress, predictability, and risk load</p>
          </CardContent>
        </Card>
        ...
        <Card className="border shadow-sm">
          <CardContent className="pt-4 pb-4 px-4">
            ...
            <p className="text-2xs text-muted-foreground">
              {metrics.sprintDonePoints}/{metrics.sprintTotalPoints} story points done
            </p>
            <p className="text-2xs text-muted-foreground">
              {activeSprint
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card className="border shadow-sm">
              <CardHeader className="pb-2">
                ...
                <p className="text-2xs text-muted-foreground">Committed vs completed points per sprint</p>
              </CardHeader>
              <CardContent>
            ...
            <Card className="border shadow-sm">
              <CardHeader className="pb-2">
                ...
                <p className="text-2xs text-muted-foreground">Ideal vs actual remaining points</p>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={280}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card className="border shadow-sm">
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
                  <span>Unestimated (active sprint)</span>
                  <Badge variant="outline">{metrics.unestimatedInSprint}</Badge>
                </div>
              </CardContent>
            </Card>
